...
# It takes as parameters the number of gene sets to be simulated (nSim), the size of each gene set (group_size),
...
source('MRS_master_for_github.R')
library('dplyr')
library('optparse')
...
perm_list<-lapply(1:nSim,function(i){
  ...
  #MRSlistgood<-sapply(MRSlist,function(x) inherits(x,'data.frame'))
